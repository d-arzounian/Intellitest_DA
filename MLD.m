...
if handles.Parametres.RSB==10000,
    set(handles.TxtAccueil,'String',strvcat('Test BMLD dans le silence','Sélectionner les paramètres puis cliquez sur Démarrer pour commencer    '));
else
    set(handles.TxtAccueil,'String',strvcat('Test BMLD dans le bruit', ' ','Sélectionner les paramètres puis cliquez sur Démarrer pour commencer    '));
end
...
Tab_gain = zeros(nb_essais_max,1); % création tableau remplis de 0 à 1 colonne et autant de ligne que le nb d'essai max
...
gainSAM = 10;        % Présentation du Sinus à un RS/B de 10dB au départ soit 90dB si stim à 80dB. 
...
if  handles.mod == 1;  % BMLD noise unmodulated
    ...
    noise_target = (amp*randn(1,duree_stand));   % Génération du bruit entre low_cf et high_cf (fixe)
    ...
    target1 = target1.*10.^(gainSAM/20); % Correctif pour faire varier l'intensité de SAM
    ...
    target2 = target2.*10.^(gainSAM/20); % Correctif pour faire varier l'intensité de SAM
...
elseif  handles.mod == 2;  % BMLD with SAM
    ...
    noise_target = noise_target ./ sqrt(mean(noise_target.^2)); % Normalisation RMS
    noise_target = noise_target ./ Scale; % Mettre le bruit au dB SPL voulu (fixe)
    noise_target = noise_target.*10.^(-rsbSAM/20); % Correctif pour faire varier l'intensité de SAM
    ...
    target1 = target1.* 10^((-1.5+3.*rand(1,1))/20);     %roving +-1.5dB
    target1 = target1 ./ Scale; % Mettre le bruit au dB SPL chosi dans intellitest
    target1 = target1.*10.^(rsbSAM/20); % Correctif pour faire varier l'intensité de SAM
    target1 = target1 + noise_target; % Addition Noise et Signal
    ...
    target2 = target2.* 10^((-1.5+3.*rand(1,1))/20);     %roving +-1.5dB
    target2 = target2 ./ Scale; % Mettre le bruit au dB SPL chosi dans intellitest
    target2 = target2.*10.^(rsbSAM/20); % Correctif pour faire varier l'intensité de SAM
    target2 = target2 + noise_target; % Addition Noise et Signal
    ...
    standard1 = standard1.* 10^((-1.5+3.*rand(1,1))/20);     %roving +-1.5dB
    standard1 = standard1 ./ Scale; % Mettre le bruit au dB SPL chosi dans intellitest
    standard1 = standard1.*10.^(rsbSAM/20); % Correctif pour faire varier l'intensité de SAM
    standard1 = standard1 + noise_target; % Addition Noise et Signal
    ...
    standard2 = standard2.* 10^((-1.5+3.*rand(1,1))/20);     %roving +-1.5dB
    standard2 = standard2 ./ Scale; % Mettre le bruit au dB SPL chosi dans intellitest
    standard2 = standard2.*10.^(rsbSAM/20); % Correctif pour faire varier l'intensité de SAM
    standard2 = standard2 + noise_target; % Addition Noise et Signal
    ...
    standard = standard.* 10^((-1.5+3.*rand(1,1))/20);     %roving +-1.5dB
    standard = standard ./ Scale; % Mettre le bruit au dB SPL chosi dans intellitest
    standard = standard.*10.^(rsbSAM/20); % Correctif pour faire varier l'intensité de SAM
    standard = standard + noise_target; % Addition Noise et Signal
    standard1= standard;
    standard2= standard;
         end
