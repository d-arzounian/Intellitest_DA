function varargout = Integration_ModDetect(varargin)
...
%%% 150 trials max
%%% Stimulus duration = 700 et 1400s
...
%%% starting m value = 0.3
%%% starting B value = 5

% INTEGRATION_MODDETECT M-file for Integration_ModDetect.fig
%      INTEGRATION_MODDETECT, by itself, creates a new INTEGRATION_MODDETECT or raises the existing
%      singleton*.
%
%      H = INTEGRATION_MODDETECT returns the handle to a new INTEGRATION_MODDETECT or the handle to
%      the existing singleton*.
%
%      INTEGRATION_MODDETECT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in INTEGRATION_MODDETECT.M with the given input arguments.
%
%      INTEGRATION_MODDETECT('Property','Value',...) creates a new INTEGRATION_MODDETECT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ModDetectGUI_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Integration_ModDetect_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Integration_ModDetect

% Last Modified by GUIDE v2.5 26-Mar-2014 14:34:08

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Integration_ModDetect_OpeningFcn, ...
                   'gui_OutputFcn',  @Integration_ModDetect_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Integration_ModDetect is made visible.
function Integration_ModDetect_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Integration_ModDetect (see VARARGIN)

% Choose default command line output for Integration_ModDetect
...
%    if handles.ampli==1;
%    handles.audiofilt=routine_CAMFIT2(handles);    
%    else handles.audiofilt=routine_CAMFIT(handles);
%    end
% end
...
% UIWAIT makes Integration_ModDetect wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Integration_ModDetect_OutputFcn(hObject, eventdata, handles) 
...
elseif get(hObject,'Value')==4, handles.mod = 3;
end
guidata(hObject, handles);
...
elseif get(hObject,'Value')==3, handles.fm=2;
...
if get(hObject,'Value')==2, handles.dt=1; % 0 look
elseif get(hObject,'Value')==3, handles.dt=2; % 1 look
elseif get(hObject,'Value')==4, handles.dt=3; % 2 looks
elseif get(hObject,'Value')==5, handles.dt=4; % 3 looks
elseif get(hObject,'Value')==6, handles.dt=5; % 4 looks
elseif get(hObject,'Value')==7, handles.dt=9; % 5 looks
end
guidata(hObject, handles);
...
duree_targ = (dt/handles.fm)*Fs;    % target duration
duree_stand = (dt/handles.fm)*Fs;   % standard duration = 1 sec
...
elseif handles.mod == 3;  % Frequency Modulation
 
    beta=deltaf./handles.fm;     % facteur beta de modulation en FM + AM interférente
    modulation_target = sin((2*pi*fp*t_stim./Fs + 2*pi*rand(1,1))+beta.*(sin(2*pi*fm*t_stim./Fs + 2*pi*rand(1,1))));
    modulation_standard = sin((2*pi*fp*t_stim./Fs + 2*pi*rand(1,1)));     
...
%     if handles.Parametres.Aud, target = filter(audiofilt,1,target);end
%     if handles.Parametres.Aud, standard = filter(audiofilt,1,standard);end
    ...
    set(handles.text2,'string','Lequel des deux sons contient des fluctuations?');
    reponse_correcte = 1;
...
handles.Seuil_geo = (prod(abs(Tab_inv(7:12)))).^(1/6);      %seuil géométrique de deltaf
handles.Seuil_arithdB =mean(Tab_invdB(7:12));          % Moyenne de deltaf
handles.STD_SeuildB = std(Tab_invdB(7:12));           % Ecart type de deltaf
handles.Seuil_geodB = (prod(abs(Tab_invdB(7:12)))).^(1/6);      %seuil géométrique de deltaf
handles.Essai=Essai;
handles.perf = round((Nb_correct_trials./Nb_trials)*100);          
...
if fm==0, handles.freqm='0.0';
elseif fm==1, handles.freqm='1.0';
elseif fm==2, handles.freqm='2.0';
elseif fm==5, handles.freqm='5.0';
...
if dt==1 , handles.dut= '1';
elseif dt==2 , handles.dut= '2';
elseif dt==3 , handles.dut= '3';    
elseif dt==4 , handles.dut= '4';
elseif dt==5 , handles.dut= '5';    
elseif dt==9 , handles.dut= '9' ;   end
