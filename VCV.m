...
if handles.Parametres.RSB==10000
    set(handles.TxtAccueil,'String',strvcat('Test Consonnes dans le silence         ', ' ','Cliquez sur DÈmarrer pour commencer    '));
else
    set(handles.TxtAccueil,'String',strvcat('Test Consonnes dans le bruit           ', ' ','Cliquez sur DÈmarrer pour commencer    '));
...
if handles.Parametres.TobiiRec
    ...
    if isempty(AllEyetrackers)
        ...
        warndlg({'Aucun oculomËtre dÈtectÈ.',... % uialert doesn't work because hObject.Visible is off?
                    'VÈrifier les branchements et que l''appareil est allumÈ,.',...
                    'ou bien lancer une mesure sans oculomÈtrie'},...
...
%         uiresume(handles.figure1);
        ...
        if any(size(AllEyetrackers)>1)
            warndlg({'Plusieurs oculomËtres ont ÈtÈ dÈtectÈs.',...
...
%         while strncmpi(handles.masker{iMasker}.name, signalname,5) 
        ...
        noise = noise ./ sqrt(mean(noise.^2)); % normalisation en √©nergie rms
    ...
    otherwise
        t=round(rand(1)*(length(handles.masker)-duration-4096));  % tirage aleatoire de la portion du fichier ssn √† utiliser comme masque
...
output = signal + (noise .* handles.Factor );  % Addition Signal et Bruit avec un rapport S/N donn√© (controle par Factor)
